...
def _prepare_obs_sequence(obs_history, sequence_length):
    """
    Prepare observation sequence for LSTM input.
    
    Args:
        obs_history: List of observations
        sequence_length: Desired sequence length
        
    Returns:
        seq_obs: numpy array of shape (sequence_length, obs_dim)
    """
    if obs_history is None or len(obs_history) == 0:
        return None
    
    # Import OBS_DIM to get correct observation dimension
    try:
        from Intersection.config import OBS_DIM
    except ImportError:
        # Fallback: try to infer from first observation
        if len(obs_history) > 0:
            first_obs = np.array(obs_history[0])
            if first_obs.ndim == 0:
                OBS_DIM = 1
            else:
                OBS_DIM = first_obs.size
        else:
            OBS_DIM = 118  # Default
    
    # Get the last sequence_length observations
    obs_slice = obs_history[-sequence_length:]
    obs_list = []
    for obs in obs_slice:
        obs_arr = np.array(obs) if not isinstance(obs, np.ndarray) else obs.copy()
        # Ensure obs_arr is 1D with correct shape
        if obs_arr.ndim == 0:
            # Scalar - this is an error, but handle it
            obs_arr = np.zeros(OBS_DIM)
        elif obs_arr.ndim == 1:
            # Already 1D, but check size
            if obs_arr.size != OBS_DIM:
                # Resize or pad to correct size
                if obs_arr.size < OBS_DIM:
                    obs_arr = np.pad(obs_arr, (0, OBS_DIM - obs_arr.size), mode='constant')
                else:
    ...
    # Stack into array - ensure correct shape
    seq_obs = np.array(obs_list)  # Shape: (len(obs_slice), OBS_DIM)
    
    # Verify shape
    if seq_obs.ndim == 1:
        # This means all observations were scalars or the array didn't stack correctly
        # Reshape to (len(obs_slice), OBS_DIM)
        if seq_obs.size == len(obs_slice) * OBS_DIM:
            seq_obs = seq_obs.reshape(len(obs_slice), OBS_DIM)
        else:
            # Something is wrong - create zeros array
            seq_obs = np.zeros((len(obs_slice), OBS_DIM))
    
    # Pad if needed
    if seq_obs.shape[0] < sequence_length:
        first_obs = seq_obs[0] if seq_obs.shape[0] > 0 else np.zeros(OBS_DIM)
        padding_count = sequence_length - seq_obs.shape[0]
        padding = np.tile(first_obs.reshape(1, -1), (padding_count, 1))
        seq_obs = np.vstack([padding, seq_obs])
    
    # Final shape check
    if seq_obs.shape != (sequence_length, OBS_DIM):
        # Force correct shape
        if seq_obs.size == sequence_length * OBS_DIM:
            seq_obs = seq_obs.reshape(sequence_length, OBS_DIM)
        else:
            # Something is seriously wrong - return zeros
            return np.zeros((sequence_length, OBS_DIM))
...
class MCTS:
    ...
    def search(
        ...
        if obs_history is not None and len(obs_history) > 0:
            ...
            seq_obs = _prepare_obs_sequence(obs_history, self.network.sequence_length)
            if seq_obs is not None:
                seq_obs_tensor = torch.FloatTensor(seq_obs).unsqueeze(0).to(self.device)  # Shape: (1, sequence_length, obs_dim)
    ...
    def _get_next_state_with_rollout(
        ...
        if action.size != 2:
            if action.size == 1:
                # If only one value, duplicate it or pad with zero
            ...
            if hasattr(env_copy, 'agents') and len(env_copy.agents) > 0:
                ...
                for i in range(num_agents):
                    if i == self.agent_id:
                        ...
                        # Ensure action is a numpy array with exactly 2 elements
                        agent_action = np.asarray(action).flatten()
                        if agent_action.size != 2:
                            if agent_action.size == 1:
                                agent_action = np.array([agent_action[0], 0.0])
                            else:
                                agent_action = agent_action[:2]
                        actions_all.append(agent_action)
                    else:
                        ...
                        if self.all_networks is not None and i < len(self.all_networks) and i < len(all_obs):
                            try:
                                ...
                                # Ensure other_action is a numpy array with exactly 2 elements
                                other_action = np.asarray(other_action).flatten()
                                if other_action.size != 2:
                                    if other_action.size == 1:
                                        other_action = np.array([other_action[0], 0.0])
                                    else:
                                        other_action = other_action[:2]
                                actions_all.append(other_action)
                            except Exception as e:
                                if self.debug_rollout:
                                    print(f"    [Warning] Failed to get action for agent {i}: {e}")
                                # Fallback: zero action (ensure it's a 2-element array)
                                actions_all.append(np.zeros(2))
                        else:
                            # Fallback: zero action (ensure it's a 2-element array)
                            actions_all.append(np.zeros(2))
                
                # Ensure all actions are properly formatted before stepping
                actions_array = []
                for act in actions_all:
                    act = np.asarray(act).flatten()
                    if act.size != 2:
                        if act.size == 1:
                            act = np.array([act[0], 0.0])
                        else:
                            act = act[:2]
                ...
                # Convert actions_all to numpy array with proper shape (num_agents, 2)
                actions_for_step = np.array(actions_all)
                if actions_for_step.ndim == 1:
                    # If 1D, reshape to (1, 2) for single agent
                    if actions_for_step.size == 2:
                        actions_for_step = actions_for_step.reshape(1, 2)
                    else:
                        actions_for_step = np.zeros((1, 2))
                elif actions_for_step.shape[1] != 2:
                    # If second dimension is not 2, fix it
                    if actions_for_step.shape[1] == 1:
                        # Pad with zeros
                        actions_for_step = np.hstack([actions_for_step, np.zeros((actions_for_step.shape[0], 1))])
                    elif actions_for_step.shape[1] > 2:
                        # Take first 2 elements
                        actions_for_step = actions_for_step[:, :2]
                
                next_obs_all, rewards, terminated, truncated, info = env_copy.step(actions_for_step)
                ...
                for step in range(1, self.rollout_depth):
                    ...
                    for i in range(num_agents):
                        if i == self.agent_id:
                            ...
                            # Ensure rollout_action is a numpy array with exactly 2 elements
                            rollout_action = np.asarray(rollout_action).flatten()
                            if rollout_action.size != 2:
                                if rollout_action.size == 1:
                                    rollout_action = np.array([rollout_action[0], 0.0])
                                else:
                                    rollout_action = rollout_action[:2]
                    ...
                    # Ensure all actions are properly formatted before stepping
                    actions_rollout_array = []
                    for act in actions_rollout:
                        act = np.asarray(act).flatten()
                        if act.size != 2:
                            if act.size == 1:
                                act = np.array([act[0], 0.0])
                                ...
                                act = act[:2]
                        actions_rollout_array.append(act)
                    actions_rollout = actions_rollout_array
                    
                    # Convert to numpy array with proper shape (num_agents, 2)
                    actions_for_rollout_step = np.array(actions_rollout)
                    if actions_for_rollout_step.ndim == 1:
                        if actions_for_rollout_step.size == 2:
                            actions_for_rollout_step = actions_for_rollout_step.reshape(1, 2)
                        else:
                            actions_for_rollout_step = np.zeros((1, 2))
                    elif actions_for_rollout_step.shape[1] != 2:
                        if actions_for_rollout_step.shape[1] == 1:
                            actions_for_rollout_step = np.hstack([actions_for_rollout_step, np.zeros((actions_for_rollout_step.shape[0], 1))])
                        elif actions_for_rollout_step.shape[1] > 2:
                            actions_for_rollout_step = actions_for_rollout_step[:, :2]
                    
                    # Step environment - CONTINUING ROLLOUT
                    # Note: Rollouts run in background without rendering
                    # Rollout output disabled - only episode-level logs are shown
                    try:
                        next_obs_all, rewards, terminated, truncated, info = env_copy.step(actions_for_rollout_step)
